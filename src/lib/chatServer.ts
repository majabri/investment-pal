...
export const chatFn = createServerFn({ method: "POST" })
  .validator((input: { messages: ChatMsg[] }) => input)
  .handler(async ({ data }): Promise<{ ok: boolean; content: string }> => {
    const openai = process.env.OPENAI_API_KEY;
    const lovable = process.env.LOVABLE_API_KEY;
    const key = openai ?? lovable;
    if (!key) {
      return { ok: false, content: "No AI key configured. Add OPENAI_API_KEY or LOVABLE_API_KEY in your hosting environment settings, then reload. Until then, use Copy Prompt + Open ChatGPT — same committee, manual transport." };
    }
    ...
    try {
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ model, messages: data.messages.slice(-20), max_tokens: 2000 }),
      });
      if (!res.ok) return { ok: false, content: `AI provider error (${res.status}). Check the key and billing, or use Copy Prompt + Open ChatGPT.` };
