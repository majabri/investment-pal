...
export interface LiveEarnings extends EarningsEvt { source: "live" | "seed"; companyName?: string; }
export interface LiveEcon extends EconEvent { source: "live" | "seed"; country?: string; time?: string; actual?: string; consensus?: string; }
...
export const getEarningsCalendarFn = createServerFn({ method: "POST" })
  .validator((i: { symbols: string[]; days?: number }) => i)
    ...
    try {
      const all = await cached(`earn:${days}`, async () => {
        ...
        for (const date of nextDays(days)) {
          const res = await fetch(`https://api.nasdaq.com/api/calendar/earnings?date=${date}`, { headers: HDRS });
          if (!res.ok) continue;
          const j = await res.json();
          for (const r of j?.data?.rows ?? []) {
            out.push({ date, symbol: String(r.symbol ?? "").toUpperCase(), time: String(r.time ?? ""), name: String(r.companyName ?? "") });
          }
        }
        return out;
      });
      ...
      return mine.map((r) => ({
        ...
        inPortfolio: true, source: "live" as const, companyName: r.name,
      }));
    } catch {
      const t = new Date().toISOString().slice(0, 10);
      return EARNINGS_EVENTS.filter((e) => e.date >= t && want.has(e.symbol))
        .map((e) => ({ ...e, source: "seed" as const }));
    }
  });

const HIGH = /fomc|fed |federal reserve|cpi|core pce|nonfarm|payroll|gdp|rate decision|unemployment/i;
  ...
  .handler(async ({ data }): Promise<LiveEcon[]> => {
    ...
    try {
      const rows = await cached(`econ:${days}`, async () => {
        const out: LiveEcon[] = [];
        for (const date of nextDays(days)) {
          const res = await fetch(`https://api.nasdaq.com/api/calendar/economicevents?date=${date}`, { headers: HDRS });
          if (!res.ok) continue;
          const j = await res.json();
          for (const r of j?.data?.rows ?? []) {
            ...
            out.push({
              ...
              source: "live", country: "US", time: String(r.gmt ?? ""),
              actual: String(r.actual ?? ""), consensus: String(r.consensus ?? ""),
            });
          }
        }
        return out;
