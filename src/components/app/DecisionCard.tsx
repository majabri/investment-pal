 ...
 *  missing field must read as missing rather than as an empty claim. */
...
function Impact({ entries }: { entries: ImpactEntry[] }) {
  if (entries.length === 0) return <NotProvided />;
  return (
          ...
          {open ? (
            <div className="space-y-4 border-t px-4 py-3">
              ...
              <Section title="Counterargument" hint="the case against this decision">
                {ev.counterargument ? <p>{ev.counterargument}</p> : <NotProvided />}
              </Section>
