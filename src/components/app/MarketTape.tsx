...
const ORDER = ["S&P 500", "Nasdaq", "Dow", "VIX", "10Y Yield", "Oil (WTI)", "Gold", "Bitcoin", "US Dollar"];
  ...
  return (
    <div className="flex h-9 items-center gap-5 overflow-x-auto whitespace-nowrap border-b bg-card/60 px-6 text-xs">
      ...
      {data && ORDER.map((name) => {
        ...
        const value = name === "10Y Yield" ? `${q.price.toFixed(2)}%`
          : name === "Bitcoin" ? `$${(q.price / 1000).toFixed(1)}K`
          : q.price.toLocaleString("en-US", { maximumFractionDigits: name === "S&P 500" || name === "Nasdaq" || name === "Dow" ? 0 : 2 });
        return (
          <span key={name} className="flex items-center gap-1.5">
            <span className="font-medium text-muted-foreground">{name}</span>
            <span className="tabular-nums text-foreground">{value}</span>
            {!isLevel && (
              <span className={cn("tabular-nums", q.changePct >= 0 ? "text-emerald-500" : "text-red-500")}>
