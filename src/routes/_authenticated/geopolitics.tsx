...
function Page() {
  ...
  return (
    <AppShell title="Geopolitics" subtitle="Live market-relevant developments from world coverage, impact-rated">
      {isLoading && <p className="text-sm text-muted-foreground">Scanning world coverage…</p>}
      {!isLoading && items.length === 0 && <p className="text-sm text-muted-foreground">Nothing market-relevant right now.</p>}
        ...
        {items.map((g) => (
          <Card key={g.link}>
            ...
            <CardContent className="text-sm">
              <a href={g.link} target="_blank" rel="noopener noreferrer" className="font-medium hover:underline">{g.title}</a>
